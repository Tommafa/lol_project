...
def main(env_config):
    ...
    if env_config["env"] == "development":
        ...
        pass
        # client = botocore.session.get_session().
    ...
    quoted = urllib.parse.quote_plus(
        db_settings["connection_string"].format(db_settings["server"],
                                                db_settings["db_name"],
                                                db_settings["password"]))
    engine = create_engine('mssql+pyodbc:///?odbc_connect={}'
                           .format(quoted), fast_executemany=True)
    ...
    summoners_dict, mini_series_dict = riot_r.\
        load_list_of_summoners(league_structure["summoners_reader"],
                               header, verbose=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("prepara un file di "
              "configurazione come da istruzioni nel Readme")
