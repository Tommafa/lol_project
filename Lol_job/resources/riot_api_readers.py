...
def build_summoners_links_per_division(
    ...
    challenger = \
        dict(
                ...
                page=1, headers=headers, verbose=verbose)
    ...
    for i, tier in enumerate(config_summoners_reader["tiers"][3:]):
        for j, division in \
                enumerate(config_summoners_reader["divisions"]):
            links_characteristics_list\
                .extend([dict(
                            base_link=config_summoners_reader["base_path"],
                            queue_type=config_summoners_reader["queue_type"],
                            tier=tier, division=division, page=1,
                            headers=headers,
                            verbose=verbose)])
...
def load_summoners_from_riot_api(base_link: str, queue_type: str, tier: str,
                                 division: str, page: int,
                                 headers: dict,
                                 verbose: bool = True) -> bytes:
    """load a list of summoners ordered by rank"""

    # fill link structure with input params
    link_for_request = "{}{}/{}/{}?page={}".format(base_link,
                                                   queue_type,
                                                   tier,
                                                   division, str(page))
    ...
    while problems_at_previous_iteration and iteration < 5:
        ...
        try:
            ...
            if verbose:
                msg = "Everything went well!" if status_code == 200 \
                    else "There was an error when handling the " \
                         "request for the following link: {}." \
                    .format(link_for_request)
            ...
            print("Unable to get url {} due to {}."
                  .format(link_for_request, e.__class__))


def load_list_of_summoners(configurations_for_summoners_reader: dict,
                           header: dict, verbose: bool = True) \
    ...
    for key in MiniSeriesDTO.schema()["properties"]:
        ...
        links_per_division = \
            build_summoners_links_per_division(
                configurations_for_summoners_reader,
                header, verbose)
